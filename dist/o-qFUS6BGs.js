...
const height = window450 ? 925 : 730;
const cW = width * 0.5;
const cH = height * 0.5;
const diamondW = window450 ? 450 : 1100;
const diamondH = window450 ? 690 : 720;
const diamondYOff = window450 ? heroDateSize * 1.1 : 0;
...
let textPosition = [
	...
	},
	{
		// "click here..."
		x: window450 ? 0 : width,
		y: height - 5 - heroSpacing - heroCentre
	},
	{	
		// "full visualisation"
		x: window450 ? 0 : width,
		y: height - 5 - heroCentre
	}
	];
...
function generateDate(_data) {
	...
	// let today = DateTime.fromFormat('2018-09-20', 'yyyy-MM-dd');
...
d3.json('./projects/first-listens/Calendar-Data_All.json')
	.then(function (data) {
		...
		svg.append('text')
			.attr('class', 'hero hero-date hero-accent sm:hero sm:hero-date')
			.html(`${date.date.day}<tspan class="hero hero-date sm:hero sm:hero-date hero-accent ordinal">${getOrdinal(date.date.day)}</tspan> ${date.date.toFormat('LLLL')}`)
			.attr('x', textPosition[0].x)
			.attr('y', textPosition[0].y)
			...
			// Edit: I've redone relative to the original origin (0, 0), because safari doesn't changing the origin of svg elements in the same way, and I couldn't figure out how to make it work
			// .attr('transform-origin', 'center')
			// .attr('transform', `skewX(-10) translate(${((fontSize * 5.625 * 0.5) - cH) * Math.tan(10 * Math.PI / 180)})`);
			.attr('transform', `skewX(-10) translate(${((fontSize * 5.625 * 0.5)) * Math.tan(10 * Math.PI / 180)})`);
		...
		svg.append('text')
			.attr('class', 'hero sm:hero')
			.html(`On this day in <tspan class='hero sm:hero hero-accent'>${date.date.year}</tspan>`)
			.attr('x', textPosition[1].x)
			.attr('y', textPosition[1].y);
			// .attr('x', width)
			// .attr('y', cDiamondY - (1.5 * heroSpacing) - (1.8965 * heroCentre));
		
		svg.append('text')
			.attr('class', 'hero sm:hero')
			.text('I listened to')
			.attr('x', textPosition[2].x)
			.attr('y', textPosition[2].y);
			// .attr('x', width)
			// .attr('y', cDiamondY - (0.5 * heroSpacing) - (0.8965 * heroCentre));
		
		svg.append('text')
			.attr('class', 'hero sm:hero')
			.html(`<tspan class='hero sm:hero hero-accent'>${count}</tspan> album${count > 1 ? 's' : ''}`)
			.attr('x', textPosition[3].x)
			.attr('y', textPosition[3].y);
			// .attr('x', width)
			// .attr('y', cDiamondY + (0.5 * heroSpacing) + (0.1035 * heroCentre));
		
		svg.append('text')
			.attr('class', 'hero sm:hero')
			.text('for the first time')
			.attr('x', textPosition[4].x)
			.attr('y', textPosition[4].y);
			// .attr('x', width)
			// .attr('y', cDiamondY + (1.5 * heroSpacing) + (1.1035 * heroCentre));
		
		svg.append('text')
			.attr('class', 'hero sm:hero')
			.html(`<a href="/projects/first-listens/" class='hero sm:hero hero-accent'>Click here</a> to view the full,`)
			.attr('x', width)
			...
			.attr('transform', `skewX(-10) translate(${(height - 5 - heroSpacing - heroCentre) * Math.tan(10 * Math.PI / 180)})`);
		...
		svg.append('text')
			...
			.attr('transform', `skewX(-10) translate(${(height - 5 - heroCentre) * Math.tan(10 * Math.PI / 180)})`);
